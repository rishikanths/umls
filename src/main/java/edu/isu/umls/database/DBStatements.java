...
public interface DBStatements {
	...
	public String SEARCH_QUERY = "SELECT CUI, STR from MRCONSO WHERE TS = 'P' AND STT='PF' AND ISPREF='Y'"
			+ " AND STR LIKE :concept";

	public String SEARCH_BY_CUI = "SELECT CUI, STR from umls.MRCONSO WHERE TS = 'P' AND STT='PF' AND ISPREF='Y'"
			+ " AND CUI = :cui";

	public String SEARCH_BY_AUI = "SELECT CUI from umls.MRCONSO WHERE AUI = :aui";

	public String SEARCH_SEMANTIC_TYPE_BY_CUI = "SELECT TUI, STY from umls.MRSTY WHERE CUI = :cui";
	...
	public String SEARCH_CONCEPT_HIERARCHY = "SELECT DISTINCT r.CUI1,c.CUI,c.STR,r.REL, r.RELA, st.TUI,st.STY"
			...
			+ " c.CUI = :cui AND c.CUI = r.CUI2 AND c.CUI = st.CUI AND r.CUI1 <> c.CUI"
	...
	public String SEARCH_SYNONYMS = "SELECT  c.str as STR, v.son as SOURCE FROM umls.MRCONSO as c, umls.MRSAB as v WHERE c.CUI = :cui "
			+ "AND c.sab = v.rsab";
	...
	public String SEARCH_CONCEPT_DEFINITION = "SELECT DISTINCT s.SON ,c.STR, d.DEF,d.SAB FROM umls.MRDEF as d, umls.MRCONSO as c, umls.MRSAB as s  "
			+ "WHERE d.CUI = c.CUI AND c.CUI = :cui AND c.TS = 'P' AND c.STT='PF' AND c.ISPREF='Y' AND d.SAB = s.RSAB";
